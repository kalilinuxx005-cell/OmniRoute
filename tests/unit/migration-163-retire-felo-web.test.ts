...
const RETIRED_PROVIDER_IDS = ["felo-web", "felo"] as const;
const CONTROL_PROVIDER = "opencode";
...
test("migration 163 retires every Felo id fail-closed and preserves audit history", async () => {
  ...
  const mixedAllowedConnectionsRaw =
    ' [ "felo-web-connection" , "felo-connection" , "opencode-connection" ] ';
  db.prepare(
    "INSERT INTO api_keys " +
      "(id, name, key, key_hash, key_prefix, allowed_connections, is_active, created_at) " +
  ...
  for (const provider of RETIRED_PROVIDER_IDS) {
    ...
    db.prepare(
      "INSERT INTO usage_history (provider, model, timestamp) " +
        "VALUES (?, 'felo-chat', datetime('now'))"
    ).run(provider);
    db.prepare(
      "INSERT INTO call_logs (id, timestamp, provider, model, status) " +
        "VALUES (?, datetime('now'), ?, 'felo-chat', 200)"
  ...
  const controlLeaseId = Number(
    db
      ...
      .run("opencode".padEnd(64, "0"), CONTROL_PROVIDER, `${CONTROL_PROVIDER}-connection`)
      .lastInsertRowid
  );
  ...
  const control = db
    .prepare("SELECT is_active FROM provider_connections WHERE id = 'opencode-connection'")
    .get() as { is_active: number };
  assert.equal(control.is_active, 1, "the independent OpenCode provider must remain active");
  ...
  assert.deepEqual(
    ...
    "an allowlist containing only Felo ids must remain non-empty and fail closed"
  );
  ...
  db.prepare(
    "INSERT INTO provider_connections " +
      ...
      "VALUES ('post-migration-felo', 'felo-web', 'apikey', 'post migration import', " +
  ...
  db.prepare(
    "INSERT INTO provider_connections " +
      "(id, provider, auth_type, name, is_active, test_status, created_at, updated_at) " +
      "VALUES ('post-migration-felo-alias', ' FELO ', 'apikey', 'post migration alias', " +
      "1, 'active', datetime('now'), datetime('now'))"
  ).run();
  const postMigrationAlias = db
    .prepare(
      "SELECT is_active, test_status, error_code, last_error_source " +
        "FROM provider_connections WHERE id = 'post-migration-felo-alias'"
    )
    .get() as {
    is_active: number;
    test_status: string;
  ...
  db.prepare(
    "UPDATE provider_connections SET provider = ' FeLo ', is_active = 1, test_status = 'active', " +
      "error_code = NULL, last_error = NULL, last_error_type = NULL, " +
      "last_error_source = NULL, last_error_at = NULL WHERE provider = 'felo'"
  ).run();
  const updateProtectedConnection = readConnectionById("felo-connection");
