...
> **TL;DR**: OmniRoute registers 351 provider IDs, with **153 provider-catalog entries marked `hasFree`**. The stricter audited free-model catalog covers **39 recurring pool keys / 452 entries** (445 active + 7 discontinued). Connect several suitable providers for broader fallback capacity; every quota, approval rule, privacy policy, and paid-overage condition still applies.
...
| Audited free-model inventory                         | **39 recurring pool keys / 452 catalog entries** | 445 active + 7 discontinued; distinct from the 351-provider catalog                       |
